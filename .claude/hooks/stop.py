...
def main():
    ...
    if os.path.exists(f'.tmp/hooked-sessions/{session_id}.json'):
        with open(f'.tmp/hooked-sessions/{session_id}.json', 'r') as f:
            state = json.load(f)
...
2. If you have already delivered the plan, please tell me "Yes! I am done!"
  - Set `./tmp/hooked-sessions/{session_id}.json` continuation count to {max_continuations} to stop further continuations.
3. If you are stuck, and does not know what to do next. Please post a comment on GitHub Issue asking for help.
  - First, set `./tmp/hooked-sessions/{session_id}.json` continuation count to {max_continuations} to stop further continuations.
