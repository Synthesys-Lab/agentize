...
output=$(acw 2>&1) && test_fail "acw should fail with no arguments"
echo "$output" | grep -qi "usage\|error" || test_fail "No arguments should show usage or error"

# Test with only cli-name - should fail
output=$(acw claude 2>&1) && test_fail "acw should fail with only cli-name"
...
output=$(acw claude claude-sonnet-4-20250514 2>&1) && test_fail "acw should fail with missing files"
...
output=$(acw claude claude-sonnet-4-20250514 /tmp/nonexistent.txt 2>&1) && test_fail "acw should fail with missing output"
...
output=$(acw unknown-provider model input.txt output.txt 2>&1)
exit_code=$?
